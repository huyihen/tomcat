...
public class ParseUrlPattern {
	...
	private void parse() {
		File[] files = new File(basePath).listFiles();//获取项目目录即 webapps下的所有项目
		
		if(files ==null || files.length<=0) {//说明没有部署项目导服务器
			return;
		}
		
		String projectName = null;//存放项目名，级当前文件夹的名字
		File webFile = null;
		...
		for(File fl:files) {
			...
			if(!webFile.exists()||!webFile.isFile()) {//如果没有或者不是一个文件，则说没有配置这个文件 不管
				continue;
			}
